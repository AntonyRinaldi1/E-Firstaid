  ...
  try {
    ...
    appointmentDoctors.innerHTML = `
      <p class="form-message">
        ${escapeHtml(error.message)} Start Apache and MySQL in XAMPP, then open http://localhost:8000/index.html.
      </p>
    `;
  }
}
...
async function loadDoctorsBySpecialization(specialization) {
  ...
  try {
    if (window.location.protocol === "file:") {
      throw new Error("Open the project through a PHP server URL, not directly as a file.");
    }
    ...
    doctorList.innerHTML = `
      ...
      <p class="empty-state">
        Start Apache and MySQL in XAMPP, then open this page from a localhost URL.
      </p>
    `;
  }
}
